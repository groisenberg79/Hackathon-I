...
class ApiRequest:
    ...
    def request_loc(city_name, country_code):
        ...
        data = response.json()
        json_string = json.dumps(data) 
        if json_string == "[]": # no matching city/country code match
             return 0
        else: 
            json_dict = json.loads(json_string)
...
# country = ApiRequest.request_loc_code('asdfas')
# print(country)



# lat_lon = ApiRequest.request_loc('Instambul', 'US')
# print(lat_lon)